...
\lstinline!i! is between zero and \lstinline!count!. We encode this as follows:
\begin{lstlisting}
void getchars(char *start, int count)
    //@ requires characters(start, count);
    //@ ensures characters(start, count);
...
precondition and a postcondition. VeriFast will then verify the loop as if it was written using a local recursive function. To see how this works, let's
