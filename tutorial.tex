...
\item There is one \emph{branch node} at each point where a symbolic execution path forks into two paths. This happens when multiple cases need to be considered in the symbolic execution; it is therefore also called a \emph{case split}. The symbolic execution of function \lstinline|main| involves one case split: symbolic execution of a \lstinline|malloc| call forks into one branch where no memory is available and therefore \lstinline|malloc| returns a null pointer, and another branch where memory is available and therefore \lstinline|malloc| allocates the requested amount of memory and returns a pointer to it. (A case split also happens when symbolically executing the \lstinline|if| statement, but since the two cases of the \lstinline|if| statement coincide with the two cases of the \lstinline|malloc| statement, no separate branch node is shown.)
