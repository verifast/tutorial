...
lemma void append_nil_lemma(ints values)
    requires true;
...
lemma void append_assoc_lemma(ints values1, ints values2, ints values3)
    requires true;
    ensures append(append(values1, values2), values3) == append(values1, append(values2, values3));
{
    switch (values1) {
        case ints_nil:
        case ints_cons(h, t):
            append_assoc_lemma(t, values2, values3);
    }
}
    ...
    //@ ensures stack(stack, reverse(values));
{
    ...
    //@ append_nil_lemma(reverse(values));
    while (n != 0)
        //@ invariant nodes(m, ?values1) &*& nodes(n, ?values2) &*& reverse(values) == append(reverse(values2), values1);
    {
        ...
        //@ append_assoc_lemma(reverse(values2tail), ints_cons(value, ints_nil), values1);
    }
    //@ open nodes(n, _);
    stack->head = m;
