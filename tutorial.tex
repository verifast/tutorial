...
If we run the program and enter \texttt{Hello}, we get back \texttt{olleH}.
...
void getchars(char *array, int count) {
    if (count > 0) {
        char c = getchar();
        *array = c;
        getchars(array + 1, count - 1);
    }
}

void putchars(char *array, int count) {
    if (count > 0) {
        char c = *array;
        putchar(c);
        putchars(array + 1, count - 1);
    }
}
...
\begin{lstlisting}
        *array = c;
\end{lstlisting}
This statement writes character \lstinline!c! into the memory location at address \lstinline!array!.
Whenever VeriFast sees such a statement, it checks that the function has permission to write to this location.
Specifically, it checks that a chunk that matches \lstinline!character(array, _)! is present in the symbolic heap.
...
must give it permission to write to the location at address \lstinline!array!, at least if parameter \lstinline!count! is greater than zero.
...
\begin{lstlisting}
    //@ requires count > 0 ? character(array, _) : true;
    //@ ensures count > 0 ? character(array, _) : true;
\end{lstlisting}
If we run VeriFast again, we see that VeriFast now accepts the assignment to \lstinline!*array!. However, it now complains at the recursive call of
...
\lstinline!array + 1!. Let's extend our contract to reflect this:
\begin{lstlisting}
    //@ requires count > 0 ? character(array, _) &*& (count > 1 ? character(array + 1, _) : true) : true;
    //@ ensures count > 0 ? character(array, _) &*& (count > 1 ? character(array + 1, _) : true) : true;
\end{lstlisting}
Alas, if we run VeriFast again, we notice VeriFast complains again at the recursive call. Indeed, if \lstinline!count! is greater than 2,
the recursive call now needs to permission to access the location at address \lstinline!array + 2!.
...
a call \lstinline!getchars(array, count)! requires permission to access the locations in the range \lstinline!array! through \lstinline!array + count - 1!.
...
replace the variable name \lstinline!array! by \lstinline!result!.
...
\begin{exercise}\label{exercise:mychars}
...
\begin{lstlisting}
predicate mychars(char *array, int count) =
    count <= 0 ? true : character(array, _) &*& mychars(array + 1, count - 1);
\end{lstlisting}
You can obtain something very close to the precondition of \lstinline!getchars! by unrolling this predicate five times, i.e. by replacing the occurrence of \lstinline!mychars!
with its definition five times. The only difference is that instead of \lstinline!false!, we get \lstinline!mychars(array + 5, count - 5)!. That is, \lstinline!mychars! doesn't stop
at 5 characters; it goes on forever.

So, let's solve Exercise~\ref{exercise:mychars} by using \lstinline!mychars! in the contracts:
\begin{lstlisting}
...
char *malloc(int count);
    //@ requires true;
    //@ ensures mychars(result, count);

void getchars(char *array, int count)
    //@ requires mychars(array, count);
    //@ ensures mychars(array, count);
{
    if (count > 0) {
        char c = getchar();
        *array = c;
...
int main() /*@ requires true; @*/ /*@ ensures true; @*/
{
    ...
    putchars(array, 100);
    putchars(array, 100);
    //@ leak mychars(array, 100);
    return 0;
}
\end{lstlisting}
This does'nt quite work yet. The problem is that VeriFast does not automatically replace a \lstinline!mychars! chunk with its definition, or vice versa. You need to do this explicitly by inserting
\lstinline!open! and \lstinline!close! ghost commands. For example, when you run VeriFast on the above program, VeriFast complains at the assignment to \lstinline!*array! in function
\lstinline!getchars! because it cannot find a chunk matching \lstinline!character(array, _)!. This chunk is in fact present; it's just that it is hidden inside the \lstinline!mychars! chunk. We need to
open up the \lstinline!mychars! chunk so that VeriFast can see the \lstinline!character! chunk that is inside of it.
...
{
    if (count > 0) {
        //@ open mychars(array, count);
        char c = getchar();
        *array = c;
        getchars(array + 1, count - 1);
        //@ close mychars(array, count);
    }
}
\end{lstlisting}
